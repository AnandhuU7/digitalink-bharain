...
import { getBlogBySlug, getRelatedBlogs, getAllBlogSlugs } from '@/lib/blogData';
...
export default function BlogSlugPage({ params }: BlogPageProps) {
  ...
  // Parse content and track elements for staggered animation
  const parseContent = (content: string) => {
    const elements: { type: 'h3' | 'p'; content: string; index: number }[] = [];
    let elementIndex = 0;
  ...
  const contentElements = entry.fullContent ? parseContent(entry.fullContent) : [];
        ...
        <div className="container max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8">
              ...
              {/* Full Content with staggered animation */}
                ...
                <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed text-justify">
                  {contentElements.map((element, index) => {
                    if (element.type === 'h3') {
                      ...
                      // Find the next h3 element to group paragraphs
              ...
              >
                ...
                <div className="flex gap-3">
                  {[
                    { href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(entry.title)}`, color: '#1DA1F2', icon: 'M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z' },
                    { href: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}`, color: '#1877F2', icon: 'M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z' },
                    { href: `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}&title=${encodeURIComponent(entry.title)}`, color: '#0A66C2', icon: 'M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z M2 9h4v12H2z M4 4a2 2 0 1 1 0 4 2 2 0 0 1 0-4z' },
                    { href: `mailto:?subject=${encodeURIComponent(entry.title)}&body=Check%20out%20this%20article`, color: '#1f2937', icon: 'M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z M22 6l-10 7L2 6' }
                  ].map((social, idx) => (
                    <motion.a
                      key={idx}
                      href={social.href}
